...
## Purpose
El sistema de inyección de dependencias del proyecto usa `Microsoft.Extensions.DependencyInjection`
(`IServiceCollection`/`IServiceProvider`) en lugar de `System.ComponentModel.Design.ServiceContainer`.
Todos los ensamblados (core y shell) registran y resuelven servicios con el mismo contenedor
estándar, habilitando inyección por constructor para la shell Avalonia (MVVM).
